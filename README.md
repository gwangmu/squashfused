...
However, at times, users inside rootless containers need to perform mount operations on the fly. A prime example is SquashFS images, which are commonly used for data archives. If they forgot to mount some SquashFS archives at container launch (which, in principle, they already can even if the container is rootless), they may want to mount additional archives at container runtime. But granting privilege to allow this at the cost of rootlessness seems too excessive. Particularly, **this is bad news for using rootless containers to contain remote users, potentially rogue programs inside, or even AI agents.**
...
Some introductory blog posts can be found [here](https://medium.com/@gwangmu/mounting-squashfs-inside-an-unprivileged-rootless-container-88e98d639f69) and [here](https://medium.com/@gwangmu/making-squashfused-less-ad-hoc-and-more-secure-aa91e0840448).
