...
However, at times, users inside rootless containers need to perform mount operations on the fly. A prime example is SquashFS images, which are commonly used for data archives. If they forgot to mount some SquashFS archives at container launch (which they already can even if the container is rootless), they may want to mount additional archives at container runtime. But granting privilege to allow this at the cost of rootlessness seems too excessive.
